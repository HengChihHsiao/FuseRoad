...
def kp_detection(lane_db, seg_db, nnet, image_root, debug=False, evaluator=None):
    model_mode = 'lane_shape'
    
    # input_size  = lane_db.configs["input_size"]  # [h w]
    input_size  = system_configs.test_seg_input_size # [h w]
    ...
    for ind in tqdm(range(0, num_images), ncols=67, desc="locating kps"):
        ...
        if debug:
            ...
            # seg_pred
            save_seg_path = os.path.join(result_dir, image_names[ind][:-4] + '_seg_pred.png')
            save_seg_pred(seg_outputs, save_seg_path)


    return 0

def testing(lane_db, seg_db, nnet, image_root, debug=False, evaluator=None):
    return globals()[system_configs.sampling_function](lane_db, seg_db, nnet, image_root, debug=debug, evaluator=evaluator)