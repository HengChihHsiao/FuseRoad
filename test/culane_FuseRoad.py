...
def kp_detection(lane_db, seg_db, nnet, result_dir, debug=False, evaluator=None, repeat=1,
    ...
    for ind in tqdm(range(0, num_images), ncols=67, desc="locating kps"):
        ...
        for scale in multi_scales:
            ...
            outputs, weights, _ = nnet.test('lane_shape', system_configs.max_iter, [images, masks])
            torch.cuda.synchronize(0)  # 0 is the GPU id
            t             = time.time() - t0
        ...
        if debug:
            ...
            if isEncAttn:
                ...
                _, label, _ = lane_db.__getitem__(ind)  # 4, 115
            ...
            if not isEncAttn and not isDecAttn:
                preds = lane_db.draw_annotation(ind, pred=results[0].cpu().numpy(), cls_pred=None, img=image)
    ...
    for seg_ind in tqdm(range(0, seg_num_images), ncols=67, desc="locating seg"):
        ...
        # print(f'seg_num_images: {seg_num_images}, seg_db_ind: {seg_db_ind}')
        ...
        # height, width = seg_image.shape[0:2]
        width, height = seg_image.size
        pad_mask      = np.zeros((height, width, 1), dtype=np.float32)
        # pad_mask = torch.zeros((height, width, 1), dtype=torch.float32)
        ...
        # resized_mask = transforms.Resize((seg_input_size[0], seg_input_size[1]))(pad_mask)

        # seg_image = cv2.resize(seg_image, (seg_input_size[1], seg_input_size[0]))
        # seg_label = cv2.resize(seg_label, (seg_input_size[1], seg_input_size[0]))
