...
class Evaluator(object):
    def __init__(self, lane_dataset, seg_dataset, exp_dir, poly_degree=3):
        self.dataset = lane_dataset
        # self.predictions = np.zeros((len(dataset.annotations), dataset.max_lanes, 4 + poly_degree))
        self.predictions = None
        self.runtimes = np.zeros(len(lane_dataset))
        self.loss = np.zeros(len(lane_dataset))
