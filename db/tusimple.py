...
class TUSIMPLE(DETECTION):
    def __init__(self, db_config, split):
        ...
        print("anno_files: {}".format(self.anno_files))
    ...
    def _extract_data(self):
        ...
        for anno_file in self.anno_files:
            ...
            for line in lines:
                ...
                seg_anno_path = os.path.join(self.root, data['raw_file'].replace('clips', 'seg_label_road_car_single_color')).replace('jpg', 'png')
                self._seg_anno_file.append(seg_anno_path)

                self._old_annotations[image_id] = {
                    'path': img_path,
                    'seg_path': seg_anno_path,
    ...
    def __getitem__(self, idx, transform=False):
        ...
        print(item['seg_path'])
        seg_anno = cv2.imread(item['seg_path'])
        print("img.shape: {}".format(img.shape))
        print("seg_anno.shape: {}".format(seg_anno.shape))
        cv2.imshow('img+seg', np.hstack((img, seg_anno)))
        cv2.waitKey(0)
