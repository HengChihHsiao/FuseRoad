...
def kp_detection(db, k_ind):
    ...
    for b_ind in range(batch_size):
        ...
        seg_anno = cv2.imread(item['seg_path'], cv2.IMREAD_GRAYSCALE)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        seg_anno = seg_anno > 0
        seg_anno = np.expand_dims(seg_anno, axis=-1)
        print("seg_anno.shape: {}".format(seg_anno.shape)) if system_configs.db_debug else None
        seg_anno = np.expand_dims(seg_anno, axis=0)
        ...
        if transform:
            ...
            _, _, mask = db.transform(image=img, line_strings=line_strings, segmentation_maps=mask)
            img, line_strings, seg_anno = db.transform(image=img, line_strings=line_strings, segmentation_maps=seg_anno)
            line_strings.clip_out_of_image_()
            new_anno = {'path': item['path'], 'seg_path': item['seg_path'], 'lanes': db.linestrings_to_lanes(line_strings)}
        ...
        seg_annos[b_ind] = np.logical_not(seg_anno[:, :, :, 0])

    seg_masks = torch.from_numpy(seg_annos)

    images   = torch.from_numpy(images)
    masks    = torch.from_numpy(masks)
    torchvision.utils.save_image(seg_masks[0]* 255, 'seg_masks.png', normalize=False) if system_configs.db_debug else None
    print("images.shape: {}".format(images.shape)) if system_configs.db_debug else None
    print("masks.shape: {}".format(masks.shape)) if system_configs.db_debug else None
    print("gt_lanes.shape: {}".format(gt_lanes[0].shape)) if system_configs.db_debug else None
    print("seg_mask.shape: {}".format(seg_masks.shape)) if system_configs.db_debug else None
